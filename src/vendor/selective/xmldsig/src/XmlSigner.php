...
final class XmlSigner
{
     ...
     * @return void
     */
    public function loadPfxFile($filename, $password): void
    {
     ...
     * @return void
     */
    public function loadPfx($pfx, $password): void
    {
     ...
     */
    private function getPrivateKeyDetailKey($type): string
    {
        $key = '';
     ...
     * @return void
     */
    public function loadPrivateKeyFile($filename, $password): void
    {
        if (!file_exists($filename)) {
            throw new XmlSignerException(sprintf('File not found: %s', $filename));
        }
     ...
     * @return void
     */
    public function loadPrivateKey($privateKey, $password): void
    {
        // Read the private key
     ...
     * @param $algorithm For example: sha1, sha224, sha256, sha384, sha512
     *
     * @throws XmlSignerException
     *
     * @return void
     */
    public function signXmlFile($filename, $outputFilename, $algorithm): void
    {
        if (!file_exists($filename)) {
            throw new XmlSignerException(sprintf('File not found: %s', $filename));
        }
     ...
     */
    public function signXml($xmlContent, $algorithm): string
    {
        if (!$this->privateKeyId) {
    ...
    public function sign($xml, $algorithm) : void
    {
        if (!$this->privateKeyId) {
            throw new XmlSignerException('No private key provided');
        }
